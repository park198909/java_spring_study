...
public class MemberJoinTest {
    ...
    @Test
    @DisplayName("userPw와 userPwRe가 일치하지 않으면 JoinValidationException발생, 비밀번호가 일치하지 않습니다. 예외문구 발송")
    ...
    @Test
    @DisplayName("userId는 6~16자리, userPw는 8자리 이상, 실패시 JoinValidationException 발생")
    void userIdUserPwLengthTest() {
        assertAll(
            ...
            () -> assertThrows(JoinValidationException.class,()-> {
                member = getMember();
                member.setUserId("user11111111111111111111111111111111111111111111111");
                joinService.join(member);
            }),
            // 비밀번호 8자리 미만 검증
            () -> {
    ...
    @Test
    @DisplayName("userId가 이미 가입된 상태라면 DuplicationUserException 발생")
    void dulicateUserIdTest() {
        // 두번 중복 가입
